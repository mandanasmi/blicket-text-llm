...
# ==
# Agent class using Chain-of-Thought prompting with an optional 'cot' flag.

class CoTPromptAgent(Agent):
    """
    Agent that can use explicit Chain-of-Thought (CoT) prompting based on a flag.
    When self.cot is True, the agent outputs a detailed step-by-step reasoning chain
    before the final answer. Otherwise, it outputs only the final command/answer.
    """
    def __init__(self, horizon, filter_actions, model, cot, temperature, system_msg_path):
    ...
    def act(self, obs, game_state):
        ...
        # Use CoT instructions if enabled.
        if self.cot:
            prompt += (
                "\n\nPlease provide a detailed chain-of-thought outlining all intermediate steps leading to your decision, "
                "and then output the final command in the format '> command'. Ensure only one command is included."
            )
        else:
            prompt += (
    ...
    def answer_tf(self, question: str, env: Optional[object] = None):
        ...
        # Use CoT instructions for True/False if enabled.
        if self.cot:
            prompt += (
